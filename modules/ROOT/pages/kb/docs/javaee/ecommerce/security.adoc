...
image::kb/docs/javaee/ecommerce/admin-console.png[title="Append the application's default URL with '/admin' to view the administration console in a browser"] 
...
image::kb/docs/javaee/ecommerce/customer-selected.png[title="Hover your mouse over a customer record and click to view customer details"] 
...
image::kb/docs/javaee/ecommerce/view-order-summary.png[title="Click 'view order summary' from a customer details display in order to view a customer's order details"] 
...
|image:kb/docs/javaee/ecommerce/projects-window.png[title="The AdminServlet forwards requests to resources within the web/admin folder"] |
...
image::kb/docs/javaee/ecommerce/form-based-authentication.png[title="Authentication and authorization take place in a four-step process using form-based authentication"]
...
image::kb/docs/javaee/ecommerce/projects-window-jsp.png[title="New JSP file nodes displayed in Projects window"]
...
image::kb/docs/javaee/ecommerce/go-to-file.png[title="Use the Go to File dialog to quickly open files in the editor"]
...
image::kb/docs/javaee/ecommerce/login-configuration.png[title="Specify security settings for the application using the IDE's web.xml visual editor"]
...
image::kb/docs/javaee/ecommerce/add-web-resource.png[title="Specify which resources need to be protected using the Add Web Resource dialog"]
...
image::kb/docs/javaee/ecommerce/edit-role-names.png[title="Specify roles to be associated with an authentication constraint"]

[start=12]
. Click OK. The role is added to the Role Name(s) text field. 

image::kb/docs/javaee/ecommerce/security-tab.png[title="Security constraints include specifying the web resource collection, and role(s) that are granted access to the collection"]
...
[start=14]
. Run the project ( image:kb/docs/javaee/ecommerce/run-project-btn.png[] ) to examine how the application now handles access to the administration console.
...
image::kb/docs/javaee/ecommerce/login-form.png[title="Unauthenticated attempts to access the administration console are redirected to the login page"]

[start=16]
. Click the 'submit' button to attempt login. You see the error page displayed. 

image::kb/docs/javaee/ecommerce/error-page.png[title="Error page is displayed when the server is unable to authenticate a user"]
...
image::kb/docs/javaee/ecommerce/groups-users-roles.png[title="Users, groups, and roles need to be set up and mapped between the application and server"]
...
image::kb/docs/javaee/ecommerce/file-realm.png[title="Expand the Configuration > Security > Realms nodes to view existing security realms"]
...
image::kb/docs/javaee/ecommerce/new-file-realm-user.png[title="Enter new user and group details in the New File Realm User panel"] 
...
image::kb/docs/javaee/ecommerce/nbuser-file-realm.png[title="All users and groups pertaining to the 'file' realm are displayed under File Users"] 
...
image::kb/docs/javaee/ecommerce/gf-admin-console-security.png[role="left", xref="image$./gf-admin-console-security.png"]
...
[start=8]
. Run the project ( image:kb/docs/javaee/ecommerce/run-project-btn.png[] ). When the application opens in the browser, attempt the access the administration console by entering the following URL into the browser's address bar:
...
image::kb/docs/javaee/ecommerce/ff-untrusted-connection.png[title="Firefox provides warnings for self-signed certificates"]
...
image::kb/docs/javaee/ecommerce/ff-confirm-security-exception.png[title="Firefox enables you to view the digital certificate before confirming the security exception"] 
...
image::kb/docs/javaee/ecommerce/gf-https.png[title="GlassFish provides a secure connection by default on port 8181"] 

Aside from the HTTPS protocol displayed in the browser's address bar, Firefox indicates that a secure connection is established with the blue background behind `localhost` in the address bar. Also, a lock ( image:kb/docs/javaee/ecommerce/firefox-lock-icon.png[] ) icon displays in the lower right corner of the browser. You can click the lock icon for secure pages to review certificate details.
...
image::kb/docs/javaee/ecommerce/network-listeners.png[title="View all HTTP listeners from the Network Listeners panel"] 
...
image::kb/docs/javaee/ecommerce/edit-network-listener.png[title="Security is enabled for the 'http-listener-2' network listener"]
...
image::kb/docs/javaee/ecommerce/add-web-resource2.png[title="Specify which resources need to be protected using the Add Web Resource dialog"]
...
image::kb/docs/javaee/ecommerce/checkout-security-constraint.png[title="Use the web.xml visual editor to specify security settings for the application"]
...
[start=9]
. Run the project ( image:kb/docs/javaee/ecommerce/run-project-btn.png[] ) to examine the application's behavior in a browser.
...
image::kb/docs/javaee/ecommerce/secure-checkout.png[title="The browser address bar indicates that a secure connection is established for customer checkout"] 

Also, in Firefox, note the lock ( image:kb/docs/javaee/ecommerce/firefox-lock-icon.png[] ) icon displayed in the lower right corner of the browser.
...
image::kb/docs/javaee/ecommerce/secure-admin.png[title="The browser address bar indicates that a secure connection is established for customer checkout"]
...
image::kb/docs/javaee/ecommerce/http-404.png[title="The browser address bar indicates that a secure connection is established for customer checkout"]
