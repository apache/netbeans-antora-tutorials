...
For more about locating and modifying the `netbeans.conf` configuration file, see xref:wiki::wiki/FaqNetbeansConf.adoc[+Netbeans Conf FAQ+].
...
*Note.* You can create a JAX-WS web service in a Maven project. Choose File > New Project (Ctrl-Shift-N on Linux and Windows, ⌘-Shift-N on MacOS) and then Maven Web Application or Maven EJB module from the Maven category. If you haven't used Maven with NetBeans before, see xref:wiki::wiki/MavenBestPractices.adoc[+Maven Best Practices+].
...
*Note.* To use the Oracle WebLogic server, xref:kb/docs/web/jsf-jpa-weblogic.adoc#01[+register the server with the IDE+]. Also, if you are using the WebLogic server, watch the screencast on xref:kb/docs/javaee/weblogic-javaee-m1-screencast.adoc[+Deploying a Web Application to Oracle WebLogic+].
...
image::kb/docs/websvc/generate-ws-ee6.png[]
...
image::kb/docs/websvc/jaxws-60-add-operation.png[]
...
image::kb/docs/websvc/design-view-with-op.png[title="Web service visual designer showing added operation"]
...
image::kb/docs/websvc/jaxws-60-source.png[] 

image::kb/docs/websvc/stateless-ejb-code1.png[]
...
image::kb/docs/websvc/jax-ws-testws.png[]
...
image::kb/docs/websvc/jax-ws-tester.png[]

The sum of the two numbers is displayed:


image::kb/docs/websvc/jax-ws-tester2.png[]
...
image::kb/docs/websvc/browse-ws.png[]

[start=4]
. Do not select a package name. Leave this field empty.

image::kb/docs/websvc/javaclient-pkg.png[]
...
image::kb/docs/websvc/jaxws-60-webclient.png[]
...
*Note:* If you want to run a JSP web application client on Oracle WebLogic, see xref:kb/docs/web/jsf-jpa-weblogic.adoc[+Running a Java Server Faces 2.0 Application on WebLogic+].
...
image::kb/docs/websvc/ws-ref-in-jsp-client.png[]
...
image::kb/docs/websvc/jax-ws-project-jsp-result.png[]
