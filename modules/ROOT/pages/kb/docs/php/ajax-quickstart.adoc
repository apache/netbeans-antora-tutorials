...
NOTE: PHP development environment is often configured using an *AMP package, depending on your operating system. This includes the PHP engine and Apache web server. For instructions on configuring your environment, see the xref:kb/docs/php.adoc[+PHP Learning Trail+].
...
image::./sample-app.png[title="Sample application displayed in browser"]
...
image::./ajax-process-flow.png[title="Ajax process flow diagram"]
...
image::./php-name-location.png[title="New PHP Project wizard - Name and Location panel"]
...
image::./php-run-config.png[role="left", link="images/php-run-config.png"]
...
image::./php-proj-win.png[title="Projects window displays MyAjaxApp project"]
...
image::./palette.png[title="Palette displaying HTML elements"] 
...
image::./php-insert-form.png[title="Insert form dialog"]
...
image::./insert-table.png[title="Insert table dialog"]
...
image:./code-completion.png[title="Ctrl-Space triggers code completion in the Source Editor"] 
...
To do so, right-click the project node in the Projects window and choose Properties. Select the *Run Configuration* category, then enter `index.html` in the Index File field. image:./php-entry-point.png[title="Specify the application's entry point in the Project Properties window"]


. Click OK to save changes and exit the Project Properties window.


. Run the project to see what it looks like in a browser. Click the Run Project ( image:./run-project-btn.png[] ) button. The `index.html` file displays in your default browser. 
image:./index-page.png[title="Run project to view its current state in browser"]
...
image::./php-javascript-options.png[title="JavaScript Options panel"]
...
image:./edit-file-name.png[title="Click on file nodes to edit names"]
...
1. Run the project to see what it looks like in a browser. Click the Run Project ( image:./run-project-btn.png[] ) button. The `index.html` file displays in your browser. 
image:./no-css.png[title="Successful deployment without stylesheet"]
...
* *Style Rule Editor:* A dialog enabling you to create rules based on classes, ID's and HTML elements, and set their position in the document hierarchy. (Create Rule ( image:./style-rule-editor-btn.png[] ) button, located in upper-left region of CSS editor toolbar)
