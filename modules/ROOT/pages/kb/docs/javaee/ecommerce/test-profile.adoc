...
image::kb/docs/javaee/ecommerce/plugins-manager.png[title="JMeter Kit is a NetBeans certified plugin available in the Plugins Manager"]
...
image::kb/docs/javaee/ecommerce/ide-installer.png[title="Plugins dependencies are automatically included during installation"]
...
image::kb/docs/javaee/ecommerce/load-generator-scripts.png[title="The Load Generator plugin enables you to access scripts from the Projects window"] 
...
image::kb/docs/javaee/ecommerce/file-wizard-load.png[title="You can filter category entries by typing the category you are searching for"]
...
image::kb/docs/javaee/ecommerce/use-case-test.png[title="JMeter test plans display under Load Testing Scripts in the Projects window"]

[start=5]
. To begin customizing the plan in JMeter, right-click the `useCaseTest.jmx` node and choose External Edit. JMeter opens. 

image::kb/docs/javaee/ecommerce/jmeter.png[title="NetBeans template plan is provided when opening JMeter"]
...
image::kb/docs/javaee/ecommerce/http-proxy-server.png[title="Configure the proxy server"] 
...
image::kb/docs/javaee/ecommerce/firefox-proxy.png[title="Specify 'localhost:9090' and remove entries in the 'No Proxy for' field"]
...
image::kb/docs/javaee/ecommerce/ff-clear-recent-history.png[title="Clear your browser's cache before proceeding with recording"]

[start=13]
. Return to the JMeter Test Plan panel (<<testPlanPanel,shown above>>) and change the values for `nb.users` and `nb.rampup` to `99`. 

image::kb/docs/javaee/ecommerce/user-defined-variables.png[title="Double-click into User Defined Variables table to modify variables"] 
...
[start=15]
. Run the project ( image:kb/docs/javaee/ecommerce/run-project-btn.png[] ) from the IDE. The project is deployed to GlassFish, and the welcome page opens in the browser.
...
image::kb/docs/javaee/ecommerce/recording-controller.png[title="Request grouping for welcome page displayed under Recording Controller"] 
...
image::kb/docs/javaee/ecommerce/recording-controller2.png[title="Requests according to use-case listed under Recording Controller"]
...
image::kb/docs/javaee/ecommerce/http-request.png[title="Configure an HTTP Request for HTTPS connection"]
...
image::kb/docs/javaee/ecommerce/test-action.png[title="Use a Test Action element to simulate a delay between user-initiated requests"]

[start=3]
. In JMeter's left column, copy (Ctrl-C; ⌘-C on Mac) the new `user` `delay` Test Action node, then paste it (Ctrl-V; ⌘-V on Mac) into each of the other Simple Controllers ( image:kb/docs/javaee/ecommerce/simple-controller-node.png[] ) that form the test plan, except for the final one (`/AffableBean/purchase`). 

When you finish, the final six Simple Controllers and their contents will look as follows. 

image::kb/docs/javaee/ecommerce/user-delay.png[title="Use copy and paste functionality to place existing elements elsewhere in your test plan"]
...
image::kb/docs/javaee/ecommerce/test-plan.png[title="Remove unnecessary items from test plan"]
...
image::kb/docs/javaee/ecommerce/summary-report.png[title="Summary Report displays results from the completed test plan"]
...
image::kb/docs/javaee/ecommerce/graph-results.png[title="Graph Results shows a graph that plots all sample times"]
...
!===
!Users !Ramp-up (seconds)  !Average (milliseconds) !Throughput !Error % 
...
|image:kb/docs/javaee/ecommerce/150-users.png[title="CPU levels when running the test with 150 users"] |image:kb/docs/javaee/ecommerce/400-users.png[title="CPU levels when running the test with 400 users"] 
|===
...
image::kb/docs/javaee/ecommerce/files-window.png[title="JMeter test plans contained in 'jmeter' folder within project"] 
...
[start=2]
. If you are running the Profiler for the first time, you need to perform a calibration step on the JDK. To do so, choose Profile > Advanced Commands > Run Profiler Calibration. For more information, refer to the NetBeans User FAQ wiki: xref:wiki::wiki/FaqProfilerCalibration.adoc[What exactly does the Profiler Calibration mean?].

[start=3]
. Click the Profile Project ( image:kb/docs/javaee/ecommerce/profile-btn.png[] ) button. When a project is profiled for the first time, its build script must be modified to enable profiling. The IDE warns you that it will modify the project's build script. 

image::kb/docs/javaee/ecommerce/enable-profiling.png[title="The project build script must be modified to enable profiling"]
...
image::kb/docs/javaee/ecommerce/monitor-application.png[title="Profiler window opens, enabling task selection and configuration"]
...
[start=8]
. In the Profiler Control Panel, click the VM Telemetry ( image:kb/docs/javaee/ecommerce/vm-telemetry-btn.png[] ) button. 
...
image::kb/docs/javaee/ecommerce/heap-popup.png[title="Hover your cursor over the graph for a real-time display of heap measurements"]

Click the Scale to Fit ( image:kb/docs/javaee/ecommerce/scale-to-fit-btn.png[] ) button above the graph to maintain a continuous view of the entire monitoring session.
...
image::kb/docs/javaee/ecommerce/profiler-options.png[title="A heap dump is taken by default if the JVM runs out of memory for the profiled application"]
...
image::kb/docs/javaee/ecommerce/take-heap-dump.png[title="To take a heap dump, choose Profile > Take Heap Dump from the main menu"]
...
image::kb/docs/javaee/ecommerce/inspect-heap.png[title="Use the Inspect panel to determine the largest objects in the heap"]
...
image::kb/docs/javaee/ecommerce/instances-view.png[title="Use the Instances view to examine class instances, their fields and references"]
...
image::kb/docs/javaee/ecommerce/saved-snapshots.png[title="Saved heap dumps can be loaded from the Profiler Control Panel"]
...
2. Ensure that the GlassFish server is running. If the server is running, a small green arrow is displayed next to the GlassFish icon ( image:kb/docs/javaee/ecommerce/gf-server-running-node.png[] ). If you need to start it, right-click the server node and choose Start.
...
* xref:front::projects/profiler/index.adoc[+NetBeans Profiler +]
...
* xref:wiki::wiki/HeapWalkerDemo.adoc[+HeapWalker Demo+]
* xref:kb/docs/java/profile-loadgenerator.adoc[+Using a Load Generator+]
...
* xref:kb/articles/nb-profiler-uncoveringleaks_pt1.adoc[+Uncovering Memory Leaks Using NetBeans Profiler+]
* xref:front::community/magazine/html/04/profiler.adoc[+Advanced Profiling: Theory in Practice with NetBeans+]
