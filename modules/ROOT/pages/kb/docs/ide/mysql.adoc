...
. To execute the query, either click the Run SQL (image:./run-sql-button.png[]) button in the task bar at the top (Ctrl-Shift-E), or right-click within the SQL Editor and choose Run Statement. The IDE generates the  ``Counselor``  table in the database, and you receive a message similar to the following in the Output window. 
