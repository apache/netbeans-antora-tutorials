...
image::kb/docs/javaee/ecommerce/execute-command.png[title="Choose Execute Command from a connection node to open the IDE's SQL editor"] 
...
[start=4]
. Click the Run SQL ( image:kb/docs/javaee/ecommerce/run-sql-btn.png[] ) button in the editor's toolbar. The script runs on your MySQL server. Tables are generated for the database, and sample data is added to the `product` and `category` tables.
...
image::kb/docs/javaee/ecommerce/connection-pool.png[title="Specify settings to create a connection pool that connects with a MySQL database"]
...
image::kb/docs/javaee/ecommerce/datasource-classname.png[title="Set the datasource classname for the connection pool"]
...
image::kb/docs/javaee/ecommerce/additional-properties.png[title="Set username, password and url to the database"]
...
image::kb/docs/javaee/ecommerce/ping-succeeded.png[title="Ping the MySQL server to determine whether the connection pool can connect"]
...
image::kb/docs/javaee/ecommerce/jdbc-resource.png[title="Specify JNDI name and connection pool to create the JDBC resource"]
