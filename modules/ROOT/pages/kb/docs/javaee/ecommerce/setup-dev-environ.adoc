...
image::kb/docs/javaee/ecommerce/ide-start-page.png[title="The NetBeans IDE Start Page"]

[start=2]
. Click the New Project ( image:kb/docs/javaee/ecommerce/new-project-btn.png[] ) button (Ctrl-Shift-N; ⌘-Shift-N on Mac) to create a new Java web project. The New Project wizard opens to guide you through the process. Under Categories choose Java Web, then under Projects choose Web Application. 

image::kb/docs/javaee/ecommerce/new-project-wzd.png[title="Create a project using the New Project wizard"]
...
image::kb/docs/javaee/ecommerce/new-project-wzd2.png[title="Specify development server, EE version, and whether to include CDI support"] 
...
image::kb/docs/javaee/ecommerce/ide-default-layout.png[role="left", xref="image$./ide-default-layout.png"]
...
1. Run the new `AffableBean` project. In the Projects window, you can do this by right-clicking the project node and choosing Run, otherwise, click the Run Project ( image:kb/docs/javaee/ecommerce/run-project-btn.png[] ) button (F6; fn-F6 on Mac) in the IDE's main toolbar. 

A browser window opens to display the project's welcome page. 

image::kb/docs/javaee/ecommerce/hello-world.png[title="Project automatically deploys to server and displays in browser"] 
...
image::kb/docs/javaee/ecommerce/navigator-ant-targets.png[title="The Navigator lists all available Ant targets for the build.xml script"]

Normal Ant targets are displayed using the general target ( image:kb/docs/javaee/ecommerce/ant-normal-icon.png[] ) icon. The _emphasized_ Ant target ( image:kb/docs/javaee/ecommerce/ant-emphasized-icon.png[] ) icon merely indicates that the target includes a description, which is displayed as a tooltip (as shown in the above image). For more information, see link:http://www.oracle.com/pls/topic/lookup?ctx=nb8000&id=NBDAG366[+Creating Java Projects+] in _Developing Applications with NetBeans IDE_.
...
image::kb/docs/javaee/ecommerce/files-window.png[title="Files window provides a directory-based view of your project"]
...
image::kb/docs/javaee/ecommerce/services-win-deployed-app.png[title="Services window displays server status, deployed applications and resources"]

*Note:* "GlassFish v3" is the default server name for NetBeans 6.8 users.

The green arrow icon on the GlassFish server node ( image:kb/docs/javaee/ecommerce/gf-server-running-node.png[] ) indicates that the server is running. The Applications folder lists all deployed applications; you can see that the `AffableBean` application has been successfully deployed.
...
image::kb/docs/javaee/ecommerce/register-mysql-server.png[title="Register a MySQL server in the IDE's Services window"] 
...
image::kb/docs/javaee/ecommerce/mysql-server-properties.png[title="Specify MySQL server settings"]
...
image::kb/docs/javaee/ecommerce/services-win-mysql.png[title="Connect to a MySQL server in the Services window"] 
...
image::kb/docs/javaee/ecommerce/create-mysql-db-dialog.png[title="Right-click the server node and choose Create Database to create a new database instance"]

[start=3]
. Click OK. When you do so, the database named `affablebean` is created, and a connection to the database is automatically established. Connections are displayed in the Services window using a connection node ( image:kb/docs/javaee/ecommerce/db-connection-node.png[] ).

*Note:* Connection nodes are persisted in the Services window. If you restart the IDE, the connection node displays with a jagged line ( image:kb/docs/javaee/ecommerce/connection-broken.png[] ), indicating that the connection is broken. To reconnect to a database, make sure that the database server is running, then right-click the node and choose Connect.
...
image::kb/docs/javaee/ecommerce/db-conn-affable-bean.png[title="Database connections contain the database's default schema and nodes for tables, views and procedures"]
...
* xref:kb/docs/java/project-setup.adoc[+Creating, Importing, and Configuring Java Projects+]
...
* xref:kb/articles/books.adoc[+NetBeans Books+]
