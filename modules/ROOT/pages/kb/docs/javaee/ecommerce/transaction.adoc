...
image::kb/docs/javaee/ecommerce/transaction-diagram.png[title="Several write actions are performed within a single transaction"]
...
1. Click the New File ( image:kb/docs/javaee/ecommerce/new-file-btn.png[] ) button in the IDE's toolbar. (Alternatively, press Ctrl-N; ⌘-N on Mac.) In the New File wizard, select the Java EE category, then select Session Bean.
...
image::kb/docs/javaee/ecommerce/ejb-wizard.png[title="Create a stateless EJB session bean using the Session Bean wizard"]
...
image::kb/docs/javaee/ecommerce/go-to-line.png[title="Use the Go to Line dialog to quickly navigate to a specific line"]
...
image::kb/docs/javaee/ecommerce/tool-tip-create-method.png[role="left", xref="image$./tool-tip-create-method.png"]
...
image::kb/docs/javaee/ecommerce/fix-all-imports.png[title="Press Ctrl-Shift-I to fix imports in a file"]
...
image::kb/docs/javaee/ecommerce/gf-error-report.png[title="Server responds with an HTTP status 500 message"]
...
image::kb/docs/javaee/ecommerce/null-pointer-exception.png[title="Click links in server output window to navigate directly to offending lines in source files"] 
...
[start=8]
. To confirm that the details have been recorded in the database, open the IDE's Services window (Ctrl-5; ⌘-5 on Mac). Locate the `affablebean` connection node. If the node appears broken ( image:kb/docs/javaee/ecommerce/connection-broken.png[] ), right-click the node and choose Connect.
...
image::kb/docs/javaee/ecommerce/customer-record.png[title="View Data menu option performs a 'select *' query on the selected table"] 
...
image::kb/docs/javaee/ecommerce/code-templates.png[title="View default code templates and define new templates in the Options window"] 
...
image::kb/docs/javaee/ecommerce/transaction-output.png[role="left", xref="image$./transaction-output.png"]
...
image::kb/docs/javaee/ecommerce/checkout-page.png[title="Upon transaction failure, the checkout page redisplays with error message"]
...
image::kb/docs/javaee/ecommerce/client-side-validation.png[title="Field validation messages appear upon clicking 'submit purchase'"]
...
image::kb/docs/javaee/ecommerce/jquery-editor-support.png[title="Press Ctrl-Space on JavaScript code to invoke jQuery documentation"]
...
image::kb/docs/javaee/ecommerce/javascript-window.png[title="Specify targetted browsers for your application in the Options window"]
...
image::kb/docs/javaee/ecommerce/ie-55.png[title="Documentation popup warns of method calls to non-compatible browser versions"]
...
image::kb/docs/javaee/ecommerce/server-side-validation.png[title="Temporarily disable JavaScript in your browser to test server-side validation"]
