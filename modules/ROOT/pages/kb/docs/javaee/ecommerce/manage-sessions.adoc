...
image::kb/docs/javaee/ecommerce/category-page.png[title="Category page includes shopping cart functionality"]
...
image::kb/docs/javaee/ecommerce/cart-page.png[title="Cart page includes shopping cart functionality"]
...
image::kb/docs/javaee/ecommerce/checkout-page.png[title="Checkout page includes shopping cart functionality"]
...
image::kb/docs/javaee/ecommerce/go-to-file-dialog.png[title="Use the Go to File dialog to quickly open project resources in the editor"]
...
image::kb/docs/javaee/ecommerce/breakpoint.png[title="Click in editor's left margin to set breakpoints"]

To toggle line numbers for the editor, right-click in the left margin and choose Show Line Numbers.


[start=6]
. Run the debugger. Click the Debug Project ( image:kb/docs/javaee/ecommerce/debug-project-btn.png[] ) button in the IDE's main toolbar. The GlassFish server starts (or restarts, if it is already running) and opens a socket on its debug port number. The application welcome page opens in your browser. 
...
image::kb/docs/javaee/ecommerce/breakpoint-suspended.png[title="Debugger suspends on breakpoints in editor"]

[start=9]
. Place your cursor on the call to `getSession()` and press Ctrl-Space to invoke the Javadoc documentation. 

image::kb/docs/javaee/ecommerce/javadoc-getsession.png[title="Press Ctrl-Space to invoke Javadoc documentation"] 
...
* *Documentation popup in the editor:* Javadoc documentation displays in a popup window when you press Ctrl-Space on a given element in the editor. You can click the External Browser ( image:kb/docs/javaee/ecommerce/external-browser-btn.png[] ) button to have the documentation open in your browser. If you want to use Ctrl-Space for code completion only, you can deactivate the documentation popup by opening the Options window (Tools > Options; NetBeans > Preferences on Mac), then selecting Editor > Code Completion. Deselect the 'Auto Popup Documentation Window' option.
...
image::kb/docs/javaee/ecommerce/javadoc-additem.png[title="Hover over methods in Navigator to view Javadoc documentation"]
...
image::kb/docs/javaee/ecommerce/session-variable.png[title="Hover your mouse over variables and expressions to determine their current values"]

[start=11]
. Click the Step Over ( image:kb/docs/javaee/ecommerce/step-over-btn.png[] ) button in the debugger toolbar located above the editor. The line is executed, and the debugger steps to the next line in the file.
...
image::kb/docs/javaee/ecommerce/session-variable-set.png[role="left", xref="image$./session-variable-set.png"]

--

In NetBeans 6.9, you can click the grey pointer ( image:kb/docs/javaee/ecommerce/grey-pointer.png[] ) in the popup to expand a list of variable values contained in the highlighted element.


[start=13]
. Click the Step Over ( image:kb/docs/javaee/ecommerce/step-over-btn.png[] ) button (F8; fn-F8 on Mac) to arrive at the `if` statement (line 154). Because you just clicked the 'add to cart' button in the browser, you know that the expression `userPath.equals("/addToCart")` should evaluate to `true`.
...
image::kb/docs/javaee/ecommerce/expression.png[title="Highlight expressions to determine their current values"]
...
image::kb/docs/javaee/ecommerce/cart-null.png[title="Cart object doesn't exist until user adds item to shopping cart"]

[start=16]
. Press F8 (fn-F8 on Mac) to step to the next line (line 160). Then, on line 160, where the `ShoppingCart` object is created, click the Step Into ( image:kb/docs/javaee/ecommerce/step-into-btn.png[] ) button. The debugger steps into the method being called. In this case, you are taken directly to the `ShoppingCart`'s constructor. 

image::kb/docs/javaee/ecommerce/cart-constructor.png[title="Step into methods to follow runtime execution to other classes"]

[start=17]
. Press Ctrl-Tab to switch back to the `ControllerServlet`. Note that the IDE provides a Call Stack ( image:kb/docs/javaee/ecommerce/call-stack-badge.png[] ) badge on line 160, indicating that the debugger is currently suspended somewhere on a method higher up in the call stack. 
...
image::kb/docs/javaee/ecommerce/variables-win-session.png[role="left", xref="image$./variables-win-session.png"]
...
image::kb/docs/javaee/ecommerce/variables-win-session-cart.png[role="left", xref="image$./variables-win-session-cart.png"]
...
image::kb/docs/javaee/ecommerce/create-watch.png[title="Create watches on variables as you step through code in a debug session"]
...
image::kb/docs/javaee/ecommerce/new-watch-dialog.png[title="Right-click variables and expressions in the editor and choose New Watch"]

A new watch is created on the `session` variable and all variables it contains. The watch is visible from the Watches window (Window > Debugging > Watches) or, if you toggle the Watches ( image:kb/docs/javaee/ecommerce/watch-btn.png[] ) button in the left margin of the Variables window, it displays in the top row of the Variables window. 
...
[start=22]
. Click the Step Into ( image:kb/docs/javaee/ecommerce/step-into-btn.png[] ) button to step into `ShoppingCart`'s `addItem` method.
...
image::kb/docs/javaee/ecommerce/variables-window-add-item.png[role="left", xref="image$./variables-window-add-item.png"]
...
image::kb/docs/javaee/ecommerce/breakpoint-jsp.png[title="You can suspend the debugger in JSP pages"]

[start=26]
. Click the Continue ( image:kb/docs/javaee/ecommerce/continue-btn.png[] ) button in the debugger toolbar. The debugger continues until execution completes, or until it reaches another breakpoint. In this case, the debugger suspends on line 86 in the header JSP fragment. 
...
image::kb/docs/javaee/ecommerce/variables-window-number-of-items.png[role="left", xref="image$./variables-window-number-of-items.png"]
...
[start=29]
. Click the Finish Session ( image:kb/docs/javaee/ecommerce/finish-session-btn.png[] ) button. The runtime finishes executing, and the debug session terminates. The browser displays a fully-rendered category page, and you can see that the shopping cart widget in the page header contains one item.
...
* ( image:kb/docs/javaee/ecommerce/step-out.png[] ) *Step Out:* Steps you out of the current method call. Executes and removes the topmost method call in your call stack.
* ( image:kb/docs/javaee/ecommerce/run-to-cursor.png[] ) *Run to Cursor:* Executes up to the line on which your cursor is placed.
* ( image:kb/docs/javaee/ecommerce/apply-code-changes.png[] ) *Apply Code Changes:* After editing a file, you can press this button so that the file is recompiled and changes are taken into account in the debug session.
* ( image:kb/docs/javaee/ecommerce/step-over-expression.png[] ) *Step Over Expression:* Enables you to view the input parameters and resulting output values of each method call within an expression. You can inspect the output values for the previous method and the input parameters for the next method in the Local Variables window. When there are no further method calls, Step Over Expression behaves like the Step Over ( image:kb/docs/javaee/ecommerce/step-over-btn.png[] ) command.
...
image::kb/docs/javaee/ecommerce/servers-win-http-monitor.png[title="Select the Enable HTTP Monitor option to activate the HTTP Monitor"]

[start=2]
. If your server is already running, you need to restart it. However, since we plan to use the debugger, and running the debugger restarts the server to communicate on a different port, just click the Debug Project ( image:kb/docs/javaee/ecommerce/debug-project-btn.png[] ) button in the IDE's main toolbar. The server restarts, a debug session begins and the application's welcome page opens in your browser. The HTTP Monitor displays in the bottom region of the IDE. 

image::kb/docs/javaee/ecommerce/http-monitor.png[title="HTTP Monitor displays by default in bottom region of the IDE"]
...
image::kb/docs/javaee/ecommerce/session-tab.png[title="Session details are displayed under the Session tab in the HTTP Monitor"] 
...
image::kb/docs/javaee/ecommerce/get-session.png[title="The getSession method exists in the JSP page's compiled servlet"]
...
[start=9]
. Click the Continue ( image:kb/docs/javaee/ecommerce/continue-btn.png[] ) button. The request finishes executing, and the category page displays in the browser with one item added to the cart.
...
Click the Ascending Sort ( image:kb/docs/javaee/ecommerce/ascending-sort-btn.png[] ) button so that the most recent records are listed at the top.
...
image::kb/docs/javaee/ecommerce/http-monitor-add-to-cart.png[role="left", xref="image$./http-monitor-add-to-cart.png"]
...
image::kb/docs/javaee/ecommerce/cookies-tab.png[title="Cookies are displayed under the Cookies tab in the HTTP Monitor"] 
...
image::kb/docs/javaee/ecommerce/headers-tab.png[title="Cookies are displayed under the Cookies tab in the HTTP Monitor"]
...
image::kb/docs/javaee/ecommerce/session-tab-add-to-cart.png[title="Session attributes are displayed under the Session tab in the HTTP Monitor"] 
...
[start=15]
. Switch to the IDE and note that the debugger is suspended on the breakpoint set in the `ControllerServlet`. Click the Step Over ( image:kb/docs/javaee/ecommerce/step-over-btn.png[] ) button so that the `session` variable is assigned to the session object.
...
[start=18]
. Click the Finish Session ( image:kb/docs/javaee/ecommerce/finish-session-btn.png[] ) button to terminate the debug session.
...
image::kb/docs/javaee/ecommerce/firefox.png[title="Temporarily disable cookies in your browser"]
...
image::kb/docs/javaee/ecommerce/compromised.png[title="The application's functionality is compromised when the client doesn't accept cookies"] 
...
image::kb/docs/javaee/ecommerce/go-to-file.png[title="The Go to File dialog enables quick navigation to project files"] 
...
image::kb/docs/javaee/ecommerce/session-timeout.png[title="Specify session time-out for the application under the web.xml's General tab"]
...
image::kb/docs/javaee/ecommerce/cart-page-session-intact.png[title="The cart page relies on a session object to display items in shopping cart"]
...
image::kb/docs/javaee/ecommerce/glassfish-error-report.png[title="NullPointerException occurs when request for expired session is received"]
...
image::kb/docs/javaee/ecommerce/gf-server-output.png[role="left", xref="image$./gf-server-output.png"]
...
image::kb/docs/javaee/ecommerce/nullpointer-exception.png[title="Error badge and tooltip indicate the location and cause of the problem"] 
...
[start=7]
. Click the New File ( image:kb/docs/javaee/ecommerce/new-file-btn.png[] ) button in the IDE's toolbar. (Alternatively, press Ctrl-N; ⌘-N on Mac.)
...
[start=15]
. Run the debugger. Click the Debug Project ( image:kb/docs/javaee/ecommerce/debug-project-btn.png[] ) button in the IDE's main toolbar.
...
image::kb/docs/javaee/ecommerce/filter-breakpoint.png[title="Set a breakpoint on the getSession method"]
...
[start=19]
. Click the Step Over ( image:kb/docs/javaee/ecommerce/step-over-btn.png[] ) button, then hover your mouse over the `session` variable. Provided that a minute hasn't passed since the previous request was sent, you'll see that the variable has been assigned to a `StandardSessionFacade`. This represents the session object for the request. 

image::kb/docs/javaee/ecommerce/session-exists.png[title="Hover your mouse over variables to determine their current value"]
...
image::kb/docs/javaee/ecommerce/session-null.png[title="Highlight expressions and hover your mouse over them to determine their current value"]
