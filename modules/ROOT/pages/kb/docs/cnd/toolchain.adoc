...
image::kb/docs/cnd/netbeans-stamp.png[title="Content on this page applies to the NetBeans IDE 8.0"]
...
|NetBeans IDE (including C/C++ support) |xref:front::download/index.adoc[Version 8.0] 
...
image::kb/docs/cnd/project.png[title="Project tree"]
...
image::kb/docs/cnd/options.png[title="Options panel"]
...
| 
|stringvalue 
a|List of supported platforms separated by comma.
...
|
|regestry 
|Windows registry key of the tool. Note that the XML tag must be spelled "regestry" although this is a mispelling.
...
Note that the XML tag must be spelled "path_patern" although this is a mispelling.
...
|
|regestry 
|Windows registry key of commands. Note that the XML tag must be spelled "regestry" although this is a mispelling. 
...
|
|path_patern 
|Regular expression that allows NetBeans IDE to find commands. Note that the XML tag must be spelled "path_patern" although this is a mispelling.

|scanner
| 
a|Name of error parser service, see <<errorhandler,Creating a Custom Compiler Error Handler>>

|
|id 
|Name of error parser service 
...
|recognizer 
|Regular expression that allows the IDE to find compiler 
a| For GNU under cygwin on Windows 
....
.*[\\/].*cygwin.*[\\/]bin[\\/]?$ 
....
