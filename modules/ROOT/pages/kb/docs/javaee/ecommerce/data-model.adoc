...
image::./workbench-home.png[role="left", link="images/workbench-home.png"]
...
1. In the default interface, begin by creating a new schema which will be used with the `AffableBean` application. Click the plus ( image:./plus-icon.png[] ) icon located to the right of the *Physical Schemata* heading. 

A new panel opens in the bottom region of the interface, enabling you to specify settings for the new schema. 
[.feature]
--

image::./workbench.png[role="left", link="images/workbench.png"]
...
1. Under the EER Diagrams heading in WorkBench, double-click the Add Diagram ( image:./add-diagram-btn.png[] ) icon. A new EER Diagram opens displaying an empty canvas. 

[tips]#'EER' stands for Enhanced Entity-Relationship.# 
[.feature]
--

image::./workbench-empty-canvas.png[role="left", link="images/workbench-empty-canvas.png"]

--

[start=2]
. Click the New Table ( image:./wb-new-table-icon.png[] ) icon located in the left margin, then hover your mouse onto the canvas and click again. A new table displays on the canvas. 
...
image::./wb-customer-table.png[role="left", link="images/wb-customer-table.png"]
...
1. In the left margin, click the 1:n Non-Identifying Relationship ( image:./one-many-btn.png[] ) button. This enables you to create a _one-to-many_ relationship.
...
[start=6]
. Click the 1:n Non-Identifying Relationship ( image:./one-many-btn.png[] ) button. In the following steps, you create a _one-to-many_ relationship between the `customer` and `customer_order` objects.
...
1. In the left margin, click the n:m Identifying Relationship ( image:./many-many-btn.png[] ) button. This enables you to create a _many-to-many_ relationship.
...
Choose View > Toggle Grid to disable the canvas grid. You can also create notes for your diagram using the New Text Object ( image:./text-object-btn.png[] ) button in the left margin.
...
1. In the IDE, open the Services window (Ctrl-5; ⌘-5 on Mac) and locate the database connection node ( image:./db-connection-node.png[] ) for the `affablebean` database instance you created in the xref:./setup-dev-environ.adoc#communicate[+previous tutorial unit+].
...
* xref:front::articles/mysql.adoc[+MySQL and NetBeans IDE+]
