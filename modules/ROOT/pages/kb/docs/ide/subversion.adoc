...
image::kb/docs/ide/svn-options-small.png[role="left", xref="image$./svn-options.png"]
...
image::kb/docs/ide/checkout-small.png[role="left", xref="image$./checkout.png"]
...
image::kb/docs/ide/import-small.png[role="left", xref="image$./import.png"]
...
--
|image:kb/docs/ide/right-ui.png[title="versioning color encoding displayed in editor's right margin"] 
...
When you are working in the Projects (Ctrl-1 on Windows/Cmd-1 on OS X), Files (Ctrl-2 on Windows/Cmd-2 on OS X), Favorites (Ctrl-3 on Windows/Cmd-3 on OS X), or Versioning windows, the IDE provides several visual features that aid in viewing status information about your files. In the example below, notice how the badge (e.g. image:kb/docs/ide/blue-badge.png[]), color of the file name, and adjacent status label, all coincide with each other to provide you with a simple but effective way to keep track of versioning information on your files: 

image::kb/docs/ide/badge-example.png[]
...
|*Blue* |image:kb/docs/ide/blue-text.png[] |Indicates that the file has been locally modified. 

|*Green* |image:kb/docs/ide/green-text.png[] |Indicates that the file has been locally added. 

|*Red* |image:kb/docs/ide/red-text.png[] |Indicates that the file contains conflicts between your local working copy and the repository's version. 

|*Gray* |image:kb/docs/ide/gray-text.png[] |Indicates that the file is ignored by Subversion and will not be included in versioning commands (e.g. Update and Commit). Files can only be made to be ignored if they have not yet been versioned. 
...
image::kb/docs/ide/file-labels.png[]
...
image::kb/docs/ide/versioning-window-small.png[role="left", xref="image$./versioning-window.png"]
...
|image:kb/docs/ide/diff.png[] |*Diff All* |Opens the Diff Viewer providing you with a side-by-side comparison of your local copies and the versions maintained in the repository. 

|image:kb/docs/ide/update.png[] |*Update All* |Updates all selected files from the repository. 

|image:kb/docs/ide/commit.png[] |*Commit All* |Enables you to commit local changes to the repository. 
|===
...
image::kb/docs/ide/versioning-right-click.png[]
...
Displays author and revision number information in the left margin of files opened in the Source Editor.
 |image:kb/docs/ide/annotations.png[] 
...
image::kb/docs/ide/history-viewer-small.png[role="left", xref="image$./history-viewer.png"]
...
image::kb/docs/ide/exclude-from-commit-small.png[role="left", xref="image$./exclude-from-commit.png"]
...
image::kb/docs/ide/revert-mods-small.png[role="left", xref="image$./revert-mods.png"]
...
image::kb/docs/ide/search-rev-small.png[role="left", xref="image$./search-rev.png"]
...
Comparing file revisions is a common task when working with versioned projects. The IDE enables you to compare revisions by using the Diff command, which is available from the right-click menu of a selected item (Subversion > Diff), as well as from the Versioning window. In the Versioning window, you can perform diffs by either double-clicking a listed file, otherwise you can click the Diff All icon (image:kb/docs/ide/diff.png[]) located in the toolbar at the top.
...
|===
|*Replace* (image:kb/docs/ide/insert.png[]): |Inserts the highlighted text from the previous revision into the current revision 

|*Move All* (image:kb/docs/ide/arrow.png[]): |Reverts the file's current revision to the state of the selected previous revision 

|*Remove* (image:kb/docs/ide/remove.png[]): |Removes the highlighted text from the current revision so that it mirrors the previous revision 
|===
...
|===
|*Local* ( image:kb/docs/ide/locally-mod.png[] ): |Displays locally modified files only 

|*Remote* ( image:kb/docs/ide/remotely-mod.png[] ): |Displays remotely modified files only 

|*Both* ( image:kb/docs/ide/both-mod.png[] ): |Displays both locally and remotely modified files 
|===
...
image::kb/docs/ide/svn-merge-small.png[role="left", xref="image$./svn-merge.png"]
...
To perform an update on all source files, you can click the Update All icon (image:kb/docs/ide/update.png[]), which displays in the toolbars located at the top of both the <<versioning,Versioning Window>>, as well as the <<comparing,Diff Viewer>>. Any changes that may have occurred in the repository are displayed in the Versioning Output window.
...
As indicated in <<badges,Badges and Color Coding>>, conflicts are displayed in the IDE with red text and are accompanied by a red badge (image:kb/docs/ide/red-badge.png[]) when viewed in the Projects, Files, or Favorites windows. When working in the Versioning window, conflicts are also indicated by a file's status:

image::kb/docs/ide/conflict-versioning-win.png[]
...
image::kb/docs/ide/conflict-resolver-small.png[role="left", xref="image$./conflict-resolver.png"]
...
* From the Versioning window or Diff Viewer, click the Commit All (image:kb/docs/ide/commit.png[]) button located in the toolbar.
...
image::kb/docs/ide/commit-dialog-small.png[role="left", xref="image$./commit-dialog.png"]
...
1. Type in a commit message in the Commit Message text area. Alternatively, click the Recent Messages ( image:kb/docs/ide/recent-msgs.png[] ) icon located in the upper right corner to view and select from a list of messages that you have previously used.
