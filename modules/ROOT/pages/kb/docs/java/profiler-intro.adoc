...
. In the window of one of the memory snapshots, click the Compute Difference button ( image:./comparesnapshots.png[title="Compare Snapshot button"] ) in the snapshot toolbar. Alternatively, choose Profile > Compare Memory Snapshots from the main menu.
