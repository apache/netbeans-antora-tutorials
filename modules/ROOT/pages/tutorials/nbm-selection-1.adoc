...
You should see the module listed under _Module Dependencies_. Click btn:[OK] to close the Project Properties dialog.
...
[start=2]
1. Right-click the `org.myorg.myeditor` package in the My Editor project, and choose _New > Window_ as done for `MiViewer`. Choose `editor` for the Windows Position and check Open on Application Start. Write `MyEditor` for Class Name Prefix.
...
[start=5]
1. Remove the lines we added to the constructor earlier, except for the call to `associateLookup(...)`. That line of the constructor should be changed as follows:
