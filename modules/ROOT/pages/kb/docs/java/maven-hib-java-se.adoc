...
image::./maven-repositories.png[title="Maven Repositories in Services window"]
...
image::./maven-project-wizard.png[title="Maven Archetypes in New Project wizard"]
...
image::./maven-project-libs.png[title="Dependencies under Libraries node in Projects window"]
...
image::./maven-artifacts-viewer.png[title="Graphs tab or Artifact Viewer showing dependencies"]
...
image::./maven-hibutil-wizard.png[title="Hibernate Util wizard"]
...
image::./mapping-pojos-wizard.png[title="Generate Hibernate Mapping Files and POJOs wizard"]
...
. Test the connection by typing  ``from Actor``  in the HQL Query Editor. Click the Run HQL Query button ( image:./run_hql_query_16.png[title="Run HQL Query button"] ) in the toolbar.
...
image::./maven-projectproperties.png[title="Actions pane of DVDStoreAdmin project properties window"]
