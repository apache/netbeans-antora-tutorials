...
.. Click the Run SQL ( image:kb/docs/javaee/ecommerce/run-sql-btn.png[] ) button in the editor's toolbar. The script runs on your MySQL server. Tables are generated for the `affablebean` database.
...
1. In the Services window (Ctrl-5; ⌘-5 on Mac), right-click the `category` table ( image:kb/docs/javaee/ecommerce/db-table-node.png[] ) node and choose View Data. The SQL editor opens and displays with a GUI representation of the `category` table in the lower region. Note that the table is empty, as no data has yet been added. 

image::kb/docs/javaee/ecommerce/category-table-empty.png[title="Use the SQL editor to view table data in the IDE"] 
...
[start=3]
. Click the Run SQL ( image:kb/docs/javaee/ecommerce/run-sql-btn.png[] ) button in the editor's toolbar. The SQL statement is executed.

[start=4]
. To confirm that the data has been added, run the `select * from category` query again. To do so, you can use the SQL History window. Click the SQL History ( image:kb/docs/javaee/ecommerce/sql-history-btn.png[] ) button in the editor's toolbar and double-click the `select * from category` entry. The SQL History window lists all SQL statements that you recently executed in the IDE.
...
1. Right-click the `product` table ( image:kb/docs/javaee/ecommerce/db-table-node.png[] ) node and choose Execute Command. Choosing the Execute Command menu option in the Services window opens the SQL editor in the IDE.
...
[start=3]
. Click the Run SQL ( image:kb/docs/javaee/ecommerce/run-sql-btn.png[] ) button in the editor's toolbar. 

*Note:* View the Output window (Ctrl-4; ⌘-4 on Mac) to see a log file containing results of the execution.


[start=4]
. Right-click the `product` table ( image:kb/docs/javaee/ecommerce/db-table-node.png[] ) node and choose View Data. You can see 16 new records listed in the table. 

image::kb/docs/javaee/ecommerce/product-table.png[title="Choose View Data from a table node's right-click menu to verify sample data"] 


=== NetBeans GUI Support for Database Tables

In the Services window, when you right-click a table ( image:kb/docs/javaee/ecommerce/db-table-node.png[] ) node and choose View Data, the IDE displays a visual representation of the table and the data it contains (as depicted in the image above). You can also use this GUI support to add, modify, and delete table data.

* *Add new records:* To add new records, click the Insert Record ( image:kb/docs/javaee/ecommerce/insert-record-btn.png[] ) button. An Insert Records dialog window displays, enabling you to enter new records. When you click OK, the new data is committed to the database, and the GUI representation of the table is automatically updated. 
...
* *Modify records:* You can make edits to existing records by double-clicking directly in table cells and modifying field entries. Modified entries display as green text. When you are finished editing data, click the Commit Record ( image:kb/docs/javaee/ecommerce/commit-record-btn.png[] ) button to commit changes to the actual database. (Similarly, click the Cancel Edits ( image:kb/docs/javaee/ecommerce/cancel-edits-btn.png[] ) button to cancel any edits you have made.
* *Delete individual records:* Click a row in the table, then click the Delete Selected Record ( image:kb/docs/javaee/ecommerce/delete-selected-record-btn.png[] ) button. You can also delete multiple rows simultaneously by holding Ctrl (⌘ on Mac) while clicking to select rows.
* *Delete all records:* Deleting all records within a table is referred to as '_truncating_' the table. Click the Truncate Table ( image:kb/docs/javaee/ecommerce/truncate-table-btn.png[] ) button to delete all records contained in the displayed table.

If the displayed data needs to be resynchronized with the actual database, you can click the Refresh Records ( image:kb/docs/javaee/ecommerce/refresh-records-btn.png[] ) button. Note that much of the above-described functionality can also be accessed from the right-click menu within the GUI editor.
...
image::kb/docs/javaee/ecommerce/jdbc-resource-gen-attributes.png[title="Create a new connection pool and data source using the JDBC Resource wizard"]
...
image::kb/docs/javaee/ecommerce/jdbc-resource-conn-pool.png[title="Create a new connection pool and data source using the JDBC Resource wizard"]
...
image::kb/docs/javaee/ecommerce/gf-server-jdbc-resources.png[title="View data sources and connection pools registered on the server"] 
...
1. Ensure that the GlassFish server is already running. In the Services window (Ctrl-5; ⌘-5 on Mac), expand the Servers node. Note the small green arrow next to the GlassFish icon ( image:kb/docs/javaee/ecommerce/gf-server-running-node.png[] ). 
...
image::kb/docs/javaee/ecommerce/ping-succeeded.png[title="Ping the connection pool to determine whether it has a physical connection to the database"] 
...
image::kb/docs/javaee/ecommerce/add-resource-ref-dialog.png[title="Specify resource properties in the Add Resource Reference dialog"]

[start=4]
. Click OK. The new resource is added under the Resource References heading. 

image::kb/docs/javaee/ecommerce/resource-reference.png[title="Create a reference to the data source for the application"] 
...
1. Create a new JSP page to test the data source. Click the New File ( image:kb/docs/javaee/ecommerce/new-file-btn.png[] ) button. (Alternatively, press Ctrl-N; ⌘-N on Mac.)
...
image::kb/docs/javaee/ecommerce/new-jsp-wzd.png[title="Enter details to name and place a new JSP page into the project"] 
...
image::kb/docs/javaee/ecommerce/db-report.png[title="Press Ctrl-Space in editor to invoke code suggestions"] 
...
image::kb/docs/javaee/ecommerce/insert-db-report.png[title="Specify the data source and SQL query to be executed"]
...
image::kb/docs/javaee/ecommerce/servers-window.png[title="Ensure that the Enable JDBC Driver Deployment option is selected"] 
...
image::kb/docs/javaee/ecommerce/output-window-mysql-driver.png[title="When JDBC Driver Deployment is enabled, the IDE deploys the MySQL driver to GlassFish when required"]
...
image::kb/docs/javaee/ecommerce/test-data-source.png[title="The JSP page contains data extracted from the 'affablebean' database"] 
...
image::kb/docs/javaee/ecommerce/add-context-param-dialog.png[title="Add initialization parameters using the Add Context Parameter dialog"]
...
image::kb/docs/javaee/ecommerce/context-parameters.png[title="Context parameters display in the web.xml interface"]
...
[start=11]
. Run the project. Click the Run Project ( image:kb/docs/javaee/ecommerce/run-project-btn.png[] ) button. The project's index page opens in the browser, and you see the values for the `categoryImagePath` and `productImagePath` context parameters displayed in the page. 

image::kb/docs/javaee/ecommerce/context-params-browser.png[title="Use 'initParam' with JSP EL delimiters to display context parameter values"]
...
image::kb/docs/javaee/ecommerce/gf-libraries.png[title="GlassFish libraries are available to your project"] 
...
image::kb/docs/javaee/ecommerce/db-query.png[title="Type 'db', then press Ctrl-Space to invoke code completion suggestions"]
...
image::kb/docs/javaee/ecommerce/insert-db-query.png[title="Specify query details to create an SQL query using JSTL <sql:query> tags"]
...
image::kb/docs/javaee/ecommerce/jstl-for-each.png[title="Type 'jstl', then press Ctrl-Space to invoke code completion suggestions"]
...
image::kb/docs/javaee/ecommerce/insert-jstl-for-each.png[title="Specify details to create a 'for each' loop using JSTL <c:forEach> tags"]
...
[start=9]
. Click the Run Project ( image:kb/docs/javaee/ecommerce/run-project-btn.png[] ) button. The project's index page opens in the browser, and you see the names and images of the four categories. 

image::kb/docs/javaee/ecommerce/index-category-images.png[title="'for each' loop extracts details from the database and utilizes them in the page"]

[start=10]
. Click any of the four images in the browser. The category page displays. 

image::kb/docs/javaee/ecommerce/category-page.png[title="Category images from index page link to category page"]
...
image::kb/docs/javaee/ecommerce/category-page-bakery.png[title="Select categories by clicking category buttons in the left column"] 
...
<c:when test="${category.id == pageContext.request.queryString}">
----
(To set a breakpoint, click in the left margin of the line. A breakpoint ( image:kb/docs/javaee/ecommerce/breakpoint-icon.png[] ) icon displays.)

[start=2]
. In the IDE's main toolbar, click the Debug Project ( image:kb/docs/javaee/ecommerce/debug-project-btn.png[] ) button. A debugging session is activated for the project, and the application's index page opens in the browser.

[start=3]
. Click the bakery category in the index page. (You know that the ID for the bakery category is `3`).

[start=4]
. Return to the IDE, and note that the debugger is suspended on the line containing the breakpoint. When suspended, the margin shows a green arrow on the breakpoint ( image:kb/docs/javaee/ecommerce/debugger-suspended-icon.png[] ), and the line displays with green background.
...
[start=6]
. Press the Finish Debugger Session ( image:kb/docs/javaee/ecommerce/finish-debugger-session-btn.png[] ) button to terminate the debugger session.
...
image::kb/docs/javaee/ecommerce/category-page-bakery-title.png[title="The name of the selected category dynamically displays above the product table"] 
...
image::kb/docs/javaee/ecommerce/category-page-bakery-product-table.png[title="Product details are dynamically displayed for the selected category"]
