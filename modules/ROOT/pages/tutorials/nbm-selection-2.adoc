...
2. Open the MyEditor.java file. If not already showing, switch to the form designer.
+
...
3. Switch to the Source view of MyEditor.java, and rewrite the constructor of the  `MyEditor`  class as shown below:
+
[source,java]
----
public MyEditor() {
...
6. Now, since the goal is one component that can display multiple `Event`s, you need a `Node` or two to display in your component. Each one will own an instance of `Event`. So, right now you'll just add the code that will create a root node for your tree view. Add the following line to the constructor:
+
[source,java]
----
...
5. The last step is to install a bit of plumbing code that will wire up your explorer manager to your TopComponent's lookup. First, delete the line
+
[source,java]
----
...
6. Modify the constructor of `MyEditor` so it looks like this:
+
[source,java]
----
public MyEditor() {
...
3. Clean and Build, and Run again.
+
